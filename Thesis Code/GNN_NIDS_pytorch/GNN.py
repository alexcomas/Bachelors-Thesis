...
class GNN(torch.nn.Module):
    ...
    def __init__(self, config, loadEpoch : int = None, loadBestEpoch=False):
        ...
        self.ckpt_path = os.path.abspath(self.config['DIRECTORIES']['logs'])+ "\\ckpt"
        ...
        if(self.resume):
            ...
            print(f"Loaded epoch: {self.loadedEpoch}")
        else:
    ...
    def forward(self, inputs):
        ...
        shape = torch.stack([
            ...
            torch.tensor(int(self.hyperparameters['node_state_dim']) - 26)
        ], dim=0)
    ...
    def train_epoch(self, gen, epoch_index, steps_per_epoch = None):
        ...
        for i in range(limit):
            ...
            progressbar.update(i+1, {'loss': running_loss/(i+1)})
        return (running_loss/limit, torch.cat(labelsResult), torch.cat(predictedResult))
    ...
    def validate_epoch(self, gen, epoch_index, validation_steps = None):
        ...
        with torch.no_grad(): 
            ...
            for i in range(limit):
                ...
                # "    {0}".format(str(i).zfill(3)) + "    {:.5f}".format(running_vall_loss/(i+1)))
                # sys.stdout.flush()
                progressbar.update(i+1, {'loss': running_vall_loss/(i+1)})
        return (running_vall_loss/limit, torch.cat(labelsResult), torch.cat(predictedResult))
    ...
    def fit(self, training_generator, evaluating_generator, steps_per_epoch = None, validation_steps = None):
        first = True
        lastLR = 0.0
        lr = self.scheduleStep(self.startingEpoch-1)
        ...
        for epoch in range(self.startingEpoch, int(self.hyperparameters['epochs'])+1):
            ...
            lastLR = lr
            lr = self.scheduleStep(epoch)
                        
            train_metrics = GNN.calculateMetrics(train_trueLabels, train_predLabels, extended=False)
            eval_metrics = GNN.calculateMetrics(eval_trueLabels, eval_predLabels, extended=False)
            ...
            if self.config['RUN_CONFIG']['sweep'] != "True":
                torch.save({
                    ...
                    'class_precision': eval_metrics['class_precision'],
                    'class_recall': eval_metrics['class_recall']
    ...
    @staticmethod
    def ExpDRCalculation(epoch, lr: float, dr: float, ds: int):
    ...
    def scheduleStep(self, epoch):
        """Calculates the learning rate according to the chosen calculation."""
        thisLR = GNN.ExpDRCalculation(epoch, float(self.hyperparameters['learning_rate']), float(self.hyperparameters['decay_rate']), int(self.hyperparameters['decay_steps']))
