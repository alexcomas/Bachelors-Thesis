...
class GNN(tf.keras.Model):
    ...
    def __init__(self, config):
        ...
        self.message_func1 = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=int(self.config['HYPERPARAMETERS']['node_state_dim'])*2 ),
        ...
        self.readout = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=int(self.config['HYPERPARAMETERS']['node_state_dim'])),
    ...
    def call(self, inputs):
        ...
        shape = tf.stack([
            n_connections,
            int(self.config['HYPERPARAMETERS']['node_state_dim']) - len(generator.chosen_connection_features)
        ], axis=0)
