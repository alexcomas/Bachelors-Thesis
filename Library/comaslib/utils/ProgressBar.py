...
class ProgressBar():
    ...
    def update(self, progress: int, extraInfo = None):
        ...
        if(not self.steps[step]):
            ...
            if (self.times[step]-self.times[self.current]) == 0:
                self.velocity[step] = sys.float_info.max
            else:
        ...
        if(step != self.initial_step):
            ...
            veloStr = f'{self.velocity[step]:.2f}'
            if self.velocity[step] >= 1e4:
                veloStr = f'{self.velocity[step]:.2e}'
            print(self.color + f"\r|{bar}| {percent:.2f}% | {veloStr} {self.unit}/s | {eta}{extraInfoString}    " + f"", end="\r")
        else:
